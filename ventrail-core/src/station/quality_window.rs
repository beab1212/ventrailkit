//! Station quality window processing primitives.
...
const CHANNEL_BIAS: u32 = 1626;
const WINDOW_LIMIT: usize = 52;

#[derive(Clone, Debug, Default)]
pub struct StationQualityWindowRecord {
    pub station: u32,
    pub channel: u16,
    pub epoch: u64,
    pub reading: i32,
...
impl StationQualityWindowRecord {
...
impl ChannelAccumulator {
    pub fn push(&mut self, rec: &StationQualityWindowRecord) {
        if self.count == 0 {
...
pub fn fold_station_quality_window(payload: &[u8]) -> ChannelAccumulator {
    let mut acc = ChannelAccumulator::default();
    let declared = read_u16_le(payload, 0) as usize;
    let mut off = 2usize;
    for _ in 0..declared.min(WINDOW_LIMIT) {
        if let Some(rec) = StationQualityWindowRecord::from_wire(payload, off) {
            acc.push(&rec);
            off += 19;
        } else {
            break;
...
pub fn analyze_station_quality_window(payload: &[u8], out: &mut ByteBuffer) -> StatusMessage {
    if payload.len() < 2 {
        return StatusMessage::fail(Status::Truncated, "station quality_window");
    }
    let acc = fold_station_quality_window(payload);
    acc.emit(out);
...
pub fn route_station_quality_window(payload: &[u8], out: &mut ByteBuffer) -> StatusMessage {
    let status = analyze_station_quality_window(payload, out);
    if !status.is_ok() {
        return status;
    }
...
pub fn ingest(payload: &[u8], out: &mut ByteBuffer) -> StatusMessage {
    route_station_quality_window(payload, out)
}

pub fn ingest_station_quality_window(payload: &[u8], out: &mut ByteBuffer) -> StatusMessage {
    ingest(payload, out)
}
