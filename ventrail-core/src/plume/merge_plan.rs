//! Plume merge plan processing primitives.
...
const CHANNEL_BIAS: u32 = 113;
const WINDOW_LIMIT: usize = 56;

#[derive(Clone, Debug, Default)]
pub struct PlumeMergePlanRecord {
    pub station: u32,
    pub channel: u16,
    pub epoch: u64,
    pub reading: i32,
...
impl ChannelAccumulator {
    pub fn push(&mut self, rec: &PlumeMergePlanRecord) {
        if self.count == 0 {
...
pub fn analyze_plume_merge_plan(payload: &[u8], out: &mut ByteBuffer) -> StatusMessage {
    if payload.len() < 2 {
        return StatusMessage::fail(Status::Truncated, "plume merge_plan");
    }
    let acc = fold_plume_merge_plan(payload);
    acc.emit(out);
...
pub fn route_plume_merge_plan(payload: &[u8], out: &mut ByteBuffer) -> StatusMessage {
    let status = analyze_plume_merge_plan(payload, out);
    if !status.is_ok() {
        return status;
    }
...
pub fn ingest(payload: &[u8], out: &mut ByteBuffer) -> StatusMessage {
    route_plume_merge_plan(payload, out)
}

pub fn ingest_plume_merge_plan(payload: &[u8], out: &mut ByteBuffer) -> StatusMessage {
    ingest(payload, out)
}
