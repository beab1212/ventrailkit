//! Reef range index processing primitives.
...
const CHANNEL_BIAS: u32 = 1184;
const WINDOW_LIMIT: usize = 56;

#[derive(Clone, Debug, Default)]
pub struct ReefRangeIndexRecord {
    pub station: u32,
    pub channel: u16,
    pub epoch: u64,
    pub reading: i32,
...
impl ChannelAccumulator {
    pub fn push(&mut self, rec: &ReefRangeIndexRecord) {
        if self.count == 0 {
...
pub fn analyze_reef_range_index(payload: &[u8], out: &mut ByteBuffer) -> StatusMessage {
    if payload.len() < 2 {
        return StatusMessage::fail(Status::Truncated, "reef range_index");
    }
    let acc = fold_reef_range_index(payload);
    acc.emit(out);
...
pub fn route_reef_range_index(payload: &[u8], out: &mut ByteBuffer) -> StatusMessage {
    let status = analyze_reef_range_index(payload, out);
    if !status.is_ok() {
        return status;
    }
...
pub fn ingest(payload: &[u8], out: &mut ByteBuffer) -> StatusMessage {
    route_reef_range_index(payload, out)
}

pub fn ingest_reef_range_index(payload: &[u8], out: &mut ByteBuffer) -> StatusMessage {
    ingest(payload, out)
}
