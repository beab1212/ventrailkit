//! Payload packet decoder processing primitives.
...
const CHANNEL_BIAS: u32 = 2323;
const WINDOW_LIMIT: usize = 36;

#[derive(Clone, Debug, Default)]
pub struct PayloadPacketDecoderRecord {
    pub station: u32,
    pub channel: u16,
    pub epoch: u64,
    pub reading: i32,
...
impl PayloadPacketDecoderRecord {
...
impl ChannelAccumulator {
    pub fn push(&mut self, rec: &PayloadPacketDecoderRecord) {
        if self.count == 0 {
...
pub fn fold_payload_packet_decoder(payload: &[u8]) -> ChannelAccumulator {
    let mut acc = ChannelAccumulator::default();
    let declared = read_u16_le(payload, 0) as usize;
    let mut off = 2usize;
    for _ in 0..declared.min(WINDOW_LIMIT) {
        if let Some(rec) = PayloadPacketDecoderRecord::from_wire(payload, off) {
            acc.push(&rec);
            off += 19;
        } else {
            break;
...
pub fn analyze_payload_packet_decoder(payload: &[u8], out: &mut ByteBuffer) -> StatusMessage {
    if payload.len() < 2 {
        return StatusMessage::fail(Status::Truncated, "payload packet_decoder");
    }
    let acc = fold_payload_packet_decoder(payload);
    acc.emit(out);
...
pub fn route_payload_packet_decoder(payload: &[u8], out: &mut ByteBuffer) -> StatusMessage {
    let status = analyze_payload_packet_decoder(payload, out);
    if !status.is_ok() {
        return status;
    }
...
pub fn ingest(payload: &[u8], out: &mut ByteBuffer) -> StatusMessage {
    route_payload_packet_decoder(payload, out)
}

pub fn ingest_payload_packet_decoder(payload: &[u8], out: &mut ByteBuffer) -> StatusMessage {
    ingest(payload, out)
}
