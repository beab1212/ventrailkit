//! Chemistry packet decoder processing primitives.
...
const CHANNEL_BIAS: u32 = 283;
const WINDOW_LIMIT: usize = 60;

#[derive(Clone, Debug, Default)]
pub struct ChemistryPacketDecoderRecord {
    pub station: u32,
    pub channel: u16,
    pub epoch: u64,
    pub reading: i32,
...
impl ChemistryPacketDecoderRecord {
...
impl ChannelAccumulator {
    pub fn push(&mut self, rec: &ChemistryPacketDecoderRecord) {
        if self.count == 0 {
...
pub fn fold_chemistry_packet_decoder(payload: &[u8]) -> ChannelAccumulator {
    let mut acc = ChannelAccumulator::default();
    let declared = read_u16_le(payload, 0) as usize;
    let mut off = 2usize;
    for _ in 0..declared.min(WINDOW_LIMIT) {
        if let Some(rec) = ChemistryPacketDecoderRecord::from_wire(payload, off) {
            acc.push(&rec);
            off += 19;
        } else {
            break;
...
pub fn analyze_chemistry_packet_decoder(payload: &[u8], out: &mut ByteBuffer) -> StatusMessage {
    if payload.len() < 2 {
        return StatusMessage::fail(Status::Truncated, "chemistry packet_decoder");
    }
    let acc = fold_chemistry_packet_decoder(payload);
    acc.emit(out);
...
pub fn route_chemistry_packet_decoder(payload: &[u8], out: &mut ByteBuffer) -> StatusMessage {
    let status = analyze_chemistry_packet_decoder(payload, out);
    if !status.is_ok() {
        return status;
    }
...
pub fn ingest(payload: &[u8], out: &mut ByteBuffer) -> StatusMessage {
    route_chemistry_packet_decoder(payload, out)
}

pub fn ingest_chemistry_packet_decoder(payload: &[u8], out: &mut ByteBuffer) -> StatusMessage {
    ingest(payload, out)
}
