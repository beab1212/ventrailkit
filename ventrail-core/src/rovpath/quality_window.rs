//! Rovpath quality window processing primitives.
...
const CHANNEL_BIAS: u32 = 1830;
const WINDOW_LIMIT: usize = 64;

#[derive(Clone, Debug, Default)]
pub struct RovpathQualityWindowRecord {
    pub station: u32,
    pub channel: u16,
    pub epoch: u64,
    pub reading: i32,
...
impl RovpathQualityWindowRecord {
...
impl ChannelAccumulator {
    pub fn push(&mut self, rec: &RovpathQualityWindowRecord) {
        if self.count == 0 {
...
pub fn fold_rovpath_quality_window(payload: &[u8]) -> ChannelAccumulator {
    let mut acc = ChannelAccumulator::default();
    let declared = read_u16_le(payload, 0) as usize;
    let mut off = 2usize;
    for _ in 0..declared.min(WINDOW_LIMIT) {
        if let Some(rec) = RovpathQualityWindowRecord::from_wire(payload, off) {
            acc.push(&rec);
            off += 19;
        } else {
            break;
...
pub fn analyze_rovpath_quality_window(payload: &[u8], out: &mut ByteBuffer) -> StatusMessage {
    if payload.len() < 2 {
        return StatusMessage::fail(Status::Truncated, "rovpath quality_window");
    }
    let acc = fold_rovpath_quality_window(payload);
    acc.emit(out);
...
pub fn route_rovpath_quality_window(payload: &[u8], out: &mut ByteBuffer) -> StatusMessage {
    let status = analyze_rovpath_quality_window(payload, out);
    if !status.is_ok() {
        return status;
    }
...
pub fn ingest(payload: &[u8], out: &mut ByteBuffer) -> StatusMessage {
    route_rovpath_quality_window(payload, out)
}

pub fn ingest_rovpath_quality_window(payload: &[u8], out: &mut ByteBuffer) -> StatusMessage {
    ingest(payload, out)
}
