//! Brine merge plan processing primitives.
...
const CHANNEL_BIAS: u32 = 997;
const WINDOW_LIMIT: usize = 48;

#[derive(Clone, Debug, Default)]
pub struct BrineMergePlanRecord {
    pub station: u32,
    pub channel: u16,
    pub epoch: u64,
    pub reading: i32,
...
impl ChannelAccumulator {
    pub fn push(&mut self, rec: &BrineMergePlanRecord) {
        if self.count == 0 {
...
pub fn analyze_brine_merge_plan(payload: &[u8], out: &mut ByteBuffer) -> StatusMessage {
    if payload.len() < 2 {
        return StatusMessage::fail(Status::Truncated, "brine merge_plan");
    }
    let acc = fold_brine_merge_plan(payload);
    acc.emit(out);
...
pub fn route_brine_merge_plan(payload: &[u8], out: &mut ByteBuffer) -> StatusMessage {
    let status = analyze_brine_merge_plan(payload, out);
    if !status.is_ok() {
        return status;
    }
...
pub fn ingest(payload: &[u8], out: &mut ByteBuffer) -> StatusMessage {
    route_brine_merge_plan(payload, out)
}

pub fn ingest_brine_merge_plan(payload: &[u8], out: &mut ByteBuffer) -> StatusMessage {
    ingest(payload, out)
}
