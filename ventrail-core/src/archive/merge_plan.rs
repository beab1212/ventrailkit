//! Archive merge plan processing primitives.
...
const CHANNEL_BIAS: u32 = 2289;
const WINDOW_LIMIT: usize = 64;

#[derive(Clone, Debug, Default)]
pub struct ArchiveMergePlanRecord {
    pub station: u32,
    pub channel: u16,
    pub epoch: u64,
    pub reading: i32,
...
impl ChannelAccumulator {
    pub fn push(&mut self, rec: &ArchiveMergePlanRecord) {
        if self.count == 0 {
...
pub fn fold_archive_merge_plan(payload: &[u8]) -> ChannelAccumulator {
    let mut acc = ChannelAccumulator::default();
    let declared = read_u16_le(payload, 0) as usize;
    let mut off = 2usize;
    for _ in 0..declared.min(WINDOW_LIMIT) {
        if let Some(rec) = ArchiveMergePlanRecord::from_wire(payload, off) {
            acc.push(&rec);
            off += 19;
        } else {
            break;
...
pub fn analyze_archive_merge_plan(payload: &[u8], out: &mut ByteBuffer) -> StatusMessage {
    if payload.len() < 2 {
        return StatusMessage::fail(Status::Truncated, "archive merge_plan");
    }
    let acc = fold_archive_merge_plan(payload);
    acc.emit(out);
...
pub fn route_archive_merge_plan(payload: &[u8], out: &mut ByteBuffer) -> StatusMessage {
    let status = analyze_archive_merge_plan(payload, out);
    if !status.is_ok() {
        return status;
    }
...
pub fn ingest(payload: &[u8], out: &mut ByteBuffer) -> StatusMessage {
    route_archive_merge_plan(payload, out)
}

pub fn ingest_archive_merge_plan(payload: &[u8], out: &mut ByteBuffer) -> StatusMessage {
    ingest(payload, out)
}
